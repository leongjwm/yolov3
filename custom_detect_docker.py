...
@torch.no_grad()
def detect(opt):
    source, weights, view_img, save_txt, imgsz, json_source, dest = opt.source, opt.weights, opt.view_img, opt.save_txt, opt.img_size, opt.json, opt.destination
    ...
    if imgCounter > 1: # if video
      ...
      out = cv2.VideoWriter((os.path.join(dest, 'OutputVideo.avi')), fourcc, 15, size)
      ...
      out.release()
        ...
        for frame in os.listdir(tempSavePath):
            ...
            destination = os.path.join(os.getcwd(), dest, frame)
            # move only files
    ...
    if json_source: # for video
        ...
        master_df.to_csv(os.path.join(dest, "OutputCSV.csv"), index=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--destination', type=str, default='data/images', help='destination')
