...
def main():
  ...
  """
  print("File extensions supported: *.jpg, *.jpeg, *.png for images, *.avi, *.mp4 for videos")
  ...
  file_name = os.path.basename(fileInput) # name of input file
  dir_name = os.path.dirname(fileInput) # name of directory of input file
  ...
  if imageBool: # image file
    copyfile(os.path.join(os.getcwd(), fileInput), os.path.join(tempPath, '{}'.format(file_name)))
  else:
    frames_to_infer = []
  ...
  if imageBool:
    os.system('python custom_detect_docker.py --source {} --weights best.pt --save-txt --save-conf --destination {}'.format(relativetempPath, dir_name))
  elif videoBool and not jsonBool:
    os.system('python custom_detect_docker.py --source {} --weights best.pt --save-txt --save-conf --destination {}'.format(relativetempPath, dir_name))
    endTime = time.time()
    print("Time taken in total: " + str(endTime-startTime))
  else:
    os.system('python custom_detect_docker.py --source {} --weights best.pt --save-txt --save-conf --json {} --destination {}'.format(relativetempPath, jsonInput, dir_name))
    endTime = time.time()
    print("Time taken in total: " + str(endTime-startTime))
